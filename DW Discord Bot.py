...
def formatar_inventario(inventario):
    ...
    for item in inventario:
        linhas.append(f"{item['item']:<33} | {item['descricao']:<44} | {item['quantidade']:^10} | {item['peso']:^4} | {item['valor']:^5}")
...
def formatar_vinculos(vinculos):
    ...
    for item in vinculos:
        linhas.append(f"{item['nome']:<33} | {item['descricao']:<72} | {item['valor']:^5}")
...
def calcular_peso_total(inventario_json):
    try:
        ...
        peso_total = sum(item.get('peso', 0)*item.get('quantidade', 0) for item in inventario)
...
@bot.tree.command(name="criar_ficha", description="Cria uma ficha vazia com um nome")
@app_commands.describe(
    nome="O nome do personagem a ser criado",    
)
async def criar_ficha(interaction: discord.Interaction, nome: str):
    ...
    if ativo is None:
        ...
        ''', (user_id, ficha_id))
        conn.commit()
        await interaction.response.send_message(f'Ficha "{nome}" criada com sucesso e marcada como ativa!')
    else:
        await interaction.response.send_message(f'Ficha "{nome}" criada com sucesso!')

@bot.tree.command(name="mudar_ficha", description="Alterna a sua ficha ativa atual pelo ID")
@app_commands.describe(
    ficha_id="O ID da ficha a ser ativada",    
)
async def mudar_ficha(interaction: discord.Interaction, ficha_id: int):
    ...
    if ficha:
        ...
        ''', (ficha_id, user_id))
        conn.commit()
        await interaction.response.send_message(f'Ficha ativa alterada para: {ficha[1]}')  # ficha[1] é o nome do personagem
    else:
        await interaction.response.send_message('Ficha não encontrada ou não pertence a você.')

@bot.tree.command(name="listar_fichas", description="Lista todas as suas fichas")
...
@app_commands.describe(
    parametro="O tipo de valor a ser mostrado",    
    valor="O novo valor"
)
@app_commands.choices(
    parametro=[
        ...
        app_commands.Choice(name="Detalhe do Alinhamento", value="alinhamento_detalhe"),
...
async def definir(interaction: discord.Interaction, parametro:str, valor:str):
...
async def mostrar(interaction: discord.Interaction, parametro:str):
    ...
    if ficha:
        valor = ficha[0]
        await interaction.response.send_message(f'O/A {parametro} da sua ficha ativa é: {valor}')
    else:
        await interaction.response.send_message('Você não possui uma ficha ativa ou nenhuma ficha está definida como ativa.')

@bot.tree.command(name="add_item", description="Adiciona um item ao inventário da sua ficha ativa")
@app_commands.describe(
    nome="Nome do item.",
...
async def add_item(interaction: discord.Interaction, nome:str, descricao:str ,quantidade:int = 1, peso:int = 1,valor:int=0 ):
    ...
    if ficha_ativa:
        ...
        inventario = json.loads(inventario_json)  # Converter de JSON para um objeto Python
        ...
        item_existente = next((item for item in inventario if item["item"].lower() == nome.lower()), None)
        
        if not item_existente:
            inventario.append(novo_item)
        else:
            opa = inventario.index(item_existente)
            inventario[inventario.index(item_existente)]["quantidade"] += quantidade
...
@app_commands.describe(
    nome_item="Nome do item a ser excluído.",
)
async def rem_item(interaction: discord.Interaction, nome_item:str):
    ...
    item_existente = next((item for item in inventario if item["item"].lower() == nome_item.lower()), None)
    
    # Se o movimento não existir, exibe uma mensagem e sai
    if not item_existente:
        await interaction.response.send_message(f"O item {nome_item} não foi encontrado")
        return
...
@app_commands.describe(
    nome_item="Nome do item a ser usado.",
)
async def usar_item(interaction: discord.Interaction, nome_item:str, quantidade: int=1):
    ...
    if match:
        ...
        item_existente['descricao'] = item_existente['descricao'].replace(f"{match.group(1)} usos",f"{nova_quantidade} usos")
    else:
        nova_quantidade = item_existente['quantidade'] - quantidade
        inventario[inventario.index(item_existente)]['quantidade'] = nova_quantidade
    ...
    await interaction.response.send_message(f"{nome} usou {quantidade} {nome_item}")
...
async def vender_item(interaction: discord.Interaction, nome_item:str, quantidade: int=1):
     # Passo 1: Recuperar os movimentos atuais da ficha
    user_id = interaction.user.id
    # Buscar a ficha do jogador pelo ID
    cursor.execute('''
    ...
    moedas = next((mov for mov in inventario if mov["nome"].lower() == 'moedas'), None)
    ...
    inventario[inventario.index(moedas)]['quantidade'] = quantidade_de_moedas + valor
    ...
    cursor.execute("SELECT nome FROM fichas WHERE id = ?", (ficha_id,))
    nome = cursor.fetchone()[0]
    await interaction.response.send_message(f"{nome} vendeu {quantidade} {nome_item}, ganhando {valor} moedas")
...
@app_commands.describe(
    nome_item="Nome do item a ser mostrado.",
)
async def mostrar_item(interaction: discord.Interaction, nome_item:str):
    ...
    # Se o movimento não existir, exibe uma mensagem e sai
    if not item_existente:
        await interaction.response.send_message(f"Item '{nome_item}' não encontrado.")
        return
...
@bot.tree.command(name="add_vinculo", description="Adiciona um Vínculo à sua ficha ativa")
@app_commands.describe(
    nome="Nome do vínculo.",
    descricao="Descrição do vínculo.",
    valor="Valor do vínculo."
)
async def add_vinculo(interaction: discord.Interaction, nome:str, descricao:str="", valor:int=1 ):
    # Recuperar o ID do jogador (supondo que interaction: discord.Interaction.user.id seja o ID do jogador)
    user_id = interaction.user.id
    ...
    if ficha_ativa:
        ...
        vinculos = json.loads(vinculos_json)  # Converter de JSON para um objeto Python
...
@app_commands.describe(
    nome_vinculo="Nome do vínculo a ser excluído.",
)
async def rem_vinc(interaction: discord.Interaction, nome_vinculo:str):
    ...
    # Passo 3: Buscar o movimento com o nome especificado
    vinculo_existente = next((mov for mov in vinculos if mov["nome"].lower() == nome_vinculo.lower()), None)
    
    # Se o movimento não existir, exibe uma mensagem e sai
    if not vinculo_existente:
        await interaction.response.send_message(f"O vínculo com {nome_vinculo} não foi encontrado")
        return
    ...
    # Commit para garantir que as mudanças sejam salvas no banco de dados
    conn.commit()
    await interaction.response.send_message(f"O vínculo com {nome_vinculo} foi excluído")

@bot.tree.command(name="mostrar_vinculo", description="Mostra um vínculo da sua ficha ativa")
@app_commands.describe(
    nome_vinculo="Nome do vínculo a ser mostrado.",
)
async def mostrar_vinculo(interaction: discord.Interaction, nome_vinculo:str):
    ...
    # Passo 3: Buscar o movimento com o nome especificado
    vinculo_existente = next((vin for vin in vinculos if vin["nome"].lower() == nome_vinculo.lower()), None)
    
    # Se o movimento não existir, exibe uma mensagem e sai
    if not vinculo_existente:
        await interaction.response.send_message(f"Vínculo '{nome_vinculo}' não encontrado.")
        return
...
@bot.tree.command(name="add_mov_roll", description="Adiciona um movimento com rolagem à sua ficha ativa")
@app_commands.describe(
...
async def add_mov_roll(interaction: discord.Interaction, nome:str, gatilho:str,atributo:str="",mod:int=0,sucesso_total:str="",sucesso_parcial:str="",fracasso:str="",detalhes:str="",critico:str=""):
    # Recuperar o ID do jogador (supondo que interaction: discord.Interaction.user.id seja o ID do jogador)
    user_id = interaction.user.id
    ...
    if ficha_ativa:
        ...
        movimentos = json.loads(movimentos_json)  # Converter de JSON para um objeto Python
        ...
        movimento_existente = next((mov for mov in movimentos if mov["nome"].lower() == nome.lower()), None)
...
async def add_mov_tex(interaction: discord.Interaction, nome:str, gatilho:str="",descricao:str=""):
    # Recuperar o ID do jogador (supondo que interaction: discord.Interaction.user.id seja o ID do jogador)
    user_id = interaction.user.id
...
@app_commands.describe(
    nome_movimento="Nome do movimento a ser mostrado.",
)
async def mostrar_mov(interaction: discord.Interaction, nome_movimento:str):
    ...
    # Se a ficha não tiver movimentos, não há nada para remover
    if not resultado or not resultado[0]:
        await interaction.response.send_message("Não há movimentos para mostrar.")
        return
    ...
    # Passo 3: Buscar o movimento com o nome especificado
    movimento_existente = next((mov for mov in movimentos if mov["nome"].lower() == nome_movimento.lower()), None)
    
    # Se o movimento não existir, exibe uma mensagem e sai
    if not movimento_existente:
        await interaction.response.send_message(f"Movimento '{nome_movimento}' não encontrado.")
        return
...
@bot.tree.command(name="rem_mov", description="Exclui um movimento da sua ficha ativa")
@app_commands.describe(
    nome_movimento="Nome do movimento a ser excluído.",
)
async def rem_mov(interaction: discord.Interaction, nome_movimento:str):
    ...
    # Se a ficha não tiver movimentos, não há nada para remover
    if not resultado or not resultado[0]:
        await interaction.response.send_message("Não há movimentos para remover.")
        return
    ...
    # Commit para garantir que as mudanças sejam salvas no banco de dados
    conn.commit()
    await interaction.response.send_message(f"O movimento {nome_movimento} foi excluído")
...
async def mostrar_ficha(interaction: discord.Interaction):
    ...
    if ficha is None:
        ...
        dados_ficha = dict(zip(colunas, ficha))  # Combinar nomes das colunas com os valores
        ...
        for key, value in dados_ficha.items():
            if key == 'inventario':
                resposta += "\n**Inventário**\n"+ formatar_inventario(value) + "\n"
            elif key =='movimentos':
                resposta += "\n**Movimentos**\n"+ formatar_movimentos(value) + "\n"
            elif key=='vinculos':
                resposta += "\n**Vínculos**\n"+ formatar_vinculos(value) + "\n"
            else:
...
@bot.tree.command(name="exportar_ficha", description="Exporta uma ficha como um arquivo json")
@app_commands.describe(
    ficha_id="ID da ficha a ser exportada",
)
async def exportar_ficha(interaction: discord.Interaction, ficha_id: int=None):
    ...
    if ficha_id == None:
        ...
        if ficha_ativa:
            ...
            await interaction.response.send_message('Você não possui uma ficha ativa e nem especificou o id da ficha a ser exportada')
            return
    ...
    if ficha:
        ...
        await interaction.response.send_message(f"A ficha {ficha[2]} foi exportada com sucesso! Aqui está o arquivo.", file=discord.File(f'ficha_{ficha[2]}.json'))
        ...
        await interaction.response.send_message(f"Nenhuma ficha encontrada com o ID {ficha_id}.")

@bot.tree.command(name="deletar_ficha", description="Deleta uma ficha do banco de dados pelo ID da ficha")
async def deletar_ficha(interaction: discord.Interaction, ficha_id: int):
    try:
        ...
        if ficha is None:
            await interaction.response.send_message(f"Ficha com ID {ficha_id} não encontrada.", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"Ficha com ID {ficha_id} foi deletada com sucesso!")
    except Exception as e:
        # Mensagem de erro
        await interaction.response.send_message(f"Ocorreu um erro ao tentar deletar a ficha: {e}", ephemeral=True)
...
async def roll(interaction: discord.Interaction, expressao: str="2d6"):
    try:
        ...
        if not match:
            await interaction.response.send_message("Expressão inválida! Use o formato XdY+Z.")
            return

        # Extrair partes da expressão
        n_dados = int(match.group(1)) if match.group(1) else 1  # Número de dados, default 1
...
async def mov(interaction: discord.Interaction, movimento: str):
    ...
    if ficha_ativa:
        ...
        movimento_especifico = next((x for x in movimentos if x["nome"].lower() == movimento.lower()), None)
        ...
        if "atributo" in movimento_especifico:
            ...
            if resultado_final <=6:
                ...
                resposta+="\nSucesso Parcial!\n"+movimento_especifico['sucesso_parcial']
            elif resultado_final <=11:
                resposta+="\nSucesso Total!\n"+movimento_especifico['sucesso_total']
...
@app_commands.describe(
   quantidade="Quantidade de xp a ser adicionada"
)
...
# @app_commands.choices(
#     opcao=[
#         app_commands.Choice(name="Opção 1", value="opcao_1"),
#         app_commands.Choice(name="Opção 2", value="opcao_2"),
#         app_commands.Choice(name="Opção 3", value="opcao_3"),
#     ]
# )
